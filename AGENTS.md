...
>
> ⚠️ **流程文档执行规则：读过的文档必须落地，不得只读不用**
>
> 所有流程文档（事故响应/发布检查清单/威胁建模/质量基线等）在对应场景下**必须执行**，而非仅阅读：
> - **发布新版本前** → 必须逐项检查 `发布检查清单.md`
> - **SEV1/SEV2 事故后** → 必须按 `事故响应.md` 处理，48h 内写复盘
> - **处理 API Key/密钥** → 必须遵守 `SECURITY.md` 中的密钥管理策略
> - **提交 PR** → 必须遵守 `CONTRIBUTING.md` 中的 PR 大小限制和流程
> - **完成任何任务** → 必须提供 `质量基线.md` 中定义的验收证据
> - **修改涉及安全的代码** → 必须参考 `威胁建模.md` 中的风险分析
>
> 任何 AI 代理在任务交付时，必须说明"已遵守哪些流程文档"。
>
...
> ⚠️ **发现问题立即修复规则：发现问题不区分来源，必须及时修复**
>
> 在开发或 CI/CD 过程中发现的任何问题（无论是否为本次变更引入），**必须及时修复**，不得拖延或推诿：
> - **Dependabot 告警** — 收到 GitHub 安全告警后，当前会话内即修复，不得留到后续
> - **Linter 警告** — CI 中出现的任何 warning 必须在该次变更中清零
> - **TypeScript 错误** — 编译错误无论来源，立即修复
> - **过期的依赖版本** — 发现即升级，不等待专门排期
> - **文档缺失/过期** — 发现后立即补充或更新，不另开任务
>
> 此规则适用于所有参与者（人类开发者 + AI Agent）。"这是之前就有的"不是免责理由。

> ⚠️ **开发环境运行规则：不杀死进程，不自动重启**
>
> 当用户的本地开发环境正在运行时，AI Agent 必须遵守：
> - **只允许更新代码** — 编辑源文件、生成/替换资源文件、修改配置
> - **不允许杀死任何正在运行的进程** — 禁止使用 `pkill`、`kill`、`killall` 等命令
> - **不允许自动重启** — 修改完成后，如果效果需要重启才能看到，**告知用户需要重启**，让用户自行操作
> - **背景中的构建/开发进程** — 如果之前的任务启动了 `cargo tauri dev` 等长期进程且仍在运行，不得自动停止它；需要重启时同样告知用户
>
> 这样做的原因是：用户的开发环境包含前端 Vite 热更新和 Tauri 的 watch 模式，大部分代码变更会自动生效，不需要重启。只有 Rust 代码变更和图标等资源变更才需要重启。让用户自己控制重启时机，避免打断工作流。
...
| 当你需要... | 必须先阅读 | 执行要求 |
|------------|-----------|---------|
| 了解项目整体设计 | → [架构总览](docs/概览/架构总览.md) | 参考 |
| 修改前端代码（React/PixiJS/Live2D） | → **[前端文档索引](docs/前端/索引.md)** | 参考 |
| 修改 Tauri 壳代码（Rust） | → **[前端文档索引](docs/前端/索引.md)** | 参考 |
| 修改 PetCore 内核代码（Go） | → **[后端文档索引](docs/后端/索引.md)** | 参考 |
| 了解架构规划 | → **[架构规划方案](docs/参考/架构规划方案.md)** | 参考 |
| 了解迁移计划 | → **[迁移规划](docs/参考/迁移规划.md)** | 参考 |
| 查看参考项目 | → **[优秀开源项目参考](docs/参考/优秀开源项目参考.md)** | 参考 |
| 创建/修改/删除文档 | → **[文档规范](docs/开发指南/文档规范.md)** | 必须执行 |
| 提交代码到 Git | → **[提交规范](docs/开发指南/提交规范.md)** | 必须执行 |
| 搭建开发环境 | → **[快速上手](docs/概览/快速上手.md)** | 参考 |
| 排查已知问题 | → **[踩坑记录](docs/踩坑/)** | 必须执行 |
| 了解代码规范/Lint | → **[代码规范](docs/开发指南/代码规范.md)** | 必须执行 |
| 了解质量基线 + 验收标准 | → **[质量基线](docs/开发指南/质量基线.md)** | **必须执行** |
| **发布新版本** | → **[发布检查清单](docs/开发指南/发布检查清单.md)** | **必须逐项检查** |
| **处理事故** | → **[事故响应](docs/开发指南/事故响应.md)** | **必须按流程处理** |
| **修改涉及安全的代码** | → **[威胁建模](docs/开发指南/威胁建模.md)** | **必须参考风险分析** |
| **密钥管理** | → **SECURITY.md** | **必须遵守密钥策略** |
| 了解贡献方式 | → `.github/CONTRIBUTING.md` | 必须执行 |
| 提交 Bug/Feature | → `.github/ISSUE_TEMPLATE/` | 必须使用模板 |
| 使用构建脚本 | → `scripts/dev.sh` / `scripts/build.sh` | 参考 |
| 使用 Makefile 命令 | → `make help` | 参考 |
| 配置 Pre-commit | → `lefthook.yml` | 必须执行 |
